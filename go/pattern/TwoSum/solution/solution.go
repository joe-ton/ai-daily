...
var (
	ErrInvalidCount   = errors.New("Invalid count of Nums for comparison")
	ErrInvalidIndices = errors.New("Invalid indices for Nums")
)
...
func (t TwoSum) Find() (indices []int, err error) {
	...
	seen := make(map[int]int) // int to idx, complement tracker
